...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.hid_to_class = delta3 * transpose(hid_output) / n;
  % fprintf('Size hid to class %f\n', size(ret.hid_to_class));
  
  error_d = transpose(model.hid_to_class) * delta3;
  % fprintf('Size error_d %f\n', size(error_d));
  d2 = ((1.0 - hid_output) .* hid_output) .* error_d;
  % fprintf('Size entropy %f\n', size(d2));

  ret.input_to_hid = d2 * transpose(data.inputs) / n;
  % fprintf('Size input to hid %f\n', size(ret.input_to_hid));
  % Add weight decays
